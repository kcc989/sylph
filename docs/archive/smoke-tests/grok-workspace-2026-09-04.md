...
A deployed retry with Grok passed provider validation and called `workspace_list_files` with directory `.`. This exposed a second defect: the tool added a slash after normalizing the root to an empty path, filtering out every file. The agent then proposed replacing the starter setup. That turn was interrupted before write permissions were approved.

Directory filtering now runs in `WorkspaceFilesystem.listWorkingFiles`, after path normalization. Tests cover root aliases, subdirectory isolation, and rejection of paths outside the workspace.
