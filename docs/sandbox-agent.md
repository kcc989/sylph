...
Failed Checkpoint Checks resume the normal coding agent automatically. The completion hook supplies the exact Checkpoint and diagnostics; the agent fixes the cause with its normal tools and runs Checks again. Three automatic continuations are allowed across the Workspace until a new User message or passing Check resets the budget. Successful Checks and production Deployments do not start automatic Turns.

The existing Workspace repair-budget table retains its stored counters during this upgrade; it now counts Check continuations. Completed Check messages use a separate durable delivery receipt. No cleanup of existing Workspace state is required.
