...
## Multiline command framing

A direct Bash reproduction identified malformed CI command framing. The runner wrapped a command as `(${command})`, placing the closing parenthesis on the heredoc terminator line for dependency repair. Bash rejected that command with an unexpected-end-of-file syntax error. Putting the wrapper parentheses on separate lines passed syntax validation and a real shell execution test covering heredoc stdout, stderr, and a trailing shell comment.

The final patch fixes the general command wrapper and retains the SDK process completion API and its timeout. The speculative status-polling change is removed. The existing stream persistence, SDK/image alignment, dependency budget, and CI stage diagnostics remain. The polling-only live attempt was not a successful dependency repair; the corrected command framing still needs deployed verification.
