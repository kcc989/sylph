...
Rebased the delivery branch onto main after PRs #51 and #52 merged, preserving their model-picker tests and release-smoke tooling. On the rebased branch, the repository test command, full typecheck, lint, and formatting check passed. The Workerd native smoke fixture passed in 2.115 seconds with 221 ms boot, including durable native tools, mock cache accounting, nonresuming Check notices, request-size rejection, and bounded native compaction recovery. The deployed evidence above predates this rebase; no new platform deployment was performed while preparing the PR.
