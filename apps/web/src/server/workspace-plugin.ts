...
export type WorkspacePluginActions = {
  ...
  preview(input: { captureEvidence?: boolean }): Promise<WorkspacePreviewResult>
    ...
    async setup(context) {
      ...
      const toolRegistration = await context.tool.transform((draft) => {
        ...
        draft.add({
          ...
          description:
            "Run install, typecheck, lint, test, and build in Cloudflare CI when requested. No Preview or browser run is started. Save changed files in a durable Checkpoint first, or reuse the current Checkpoint when the working copy is clean.",
        ...
        draft.add({
          ...
          description:
            "Create or find a Preview when requested, without running lint or tests. Browser evidence is captured only when captureEvidence is true.",
          input: WorkspacePreviewToolJsonSchema,
          options: workspaceToolOptions,
          async execute(input) {
            const decoded = await decodeWorkspacePreviewToolInput(input)
            return { content: JSON.stringify(await actions.preview(decoded)) }
          },
        })
