...
const list = (directory = '') => fs.readdirSync(directory || '.', { withFileTypes: true }).flatMap(entry => {
  ...
  if (entry.isDirectory()) return ['node_modules', '.git', '.cache'].includes(entry.name) ? [] : list(path);
...
execFileSync('bun', ['install', '--lockfile-only', '--frozen-lockfile', '--ignore-scripts'], { stdio: 'inherit' });
