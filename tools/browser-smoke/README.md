...
The suite first tries required screenshot capture across freeze/reconnect and records any capture limitation. It then explicitly configures a User policy for DOM-only journey proof, with an audit reason. These journey results verify DOM behavior and viewport dimensions, not responsive screenshots or visual appearance. The initial screenshot remains separate evidence. Capture diagnostics can reproduce the platform lifecycle behavior independently of Sylph's guard.
