...
The prepared starter labels its `<prefix>-recovery` plan entry with this purpose. Recovery must exclude that database from application restore/export replacement and preserve its writer gate, manifests and secret versions. Existing control resources need independently established claims; generic application adoption deliberately cannot claim them. Coordinated recovery supports guarded D1 and R2 state, [managed KV and Queue journals](../../packages/cloudflare-recovery/MANAGED-STATE.md), and [registered SQLite Durable Objects](../../packages/cloudflare-recovery/README.md#registered-durable-object-recovery). Applications must use the required wrappers and pass their restore tests. These adapters do not recover arbitrary native KV, Queue transport backlog, arbitrary Durable Object state, or Workflow execution state.

Deploy the initial database migration `0001_initial.sql` through Alchemy into a fresh database. It includes resource claims and lifecycle review storage. `alchemy.run.ts` now provisions `ResourceToken` or uses a configured `RESOURCE_TOKEN`, scoped to Worker scripts, D1, KV, R2 and Queue permissions plus read-only Container attachment inspection in the installation account. Resource inspection and maintenance use it. Installation services retain account credentials. Project deployment commands receive only a short-lived capability tied to the exact active resource plan; broker requests verify the claim and operation on every call.

The external template includes the deployment broker state helper, coordinated D1/R2 release hooks, isolated drills, encrypted snapshots and application restore/undo. The active release is pinned in `packages/domain/src/template-release.ts`. `template-candidate.json` records a separate candidate and its source hashes.
...
Read the active built-in pin from [template-release.ts](../../packages/domain/src/template-release.ts). The `template-candidate.json` in this directory records an earlier candidate at `6450aa6b0bcd19873d09c2471ad9a359ed307a4f`; it is not the selected release or live publication status. Use [the template guide](../../docs/project-templates.md) for the current import and release contract.
