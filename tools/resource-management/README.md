...
3. Apply migration `0024_project_resources.sql` and deploy both the Website and WorkspaceRuntime through `alchemy.run.ts`. The runtime includes the `ResourceMaintenance` Workflow, and the Website receives its binding. Do not run production deployment or resource destruction without approval.
