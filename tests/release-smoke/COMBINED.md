...
Browser controls and all actions are integrated in the single Sylph PR. Read the current template pin from `packages/domain/src/template-release.ts` and verify that exact immutable commit is published before deployment. The obsolete starter 0.2.0 does not satisfy this source contract. Run from a committed, clean combined source and record that exact commit. The follow-up Browser Run fixture passed screenshot-required desktop/mobile journeys and coordinate actions at source a91e848909ec9a093c09482ae990317eeeac9738. See docs/verification/browser-service-followup-smoke.json. This separate service fixture does not substitute for combined product proof.
